...
In this guide, we covered building a `Box` component with `uinix-ui" />.  Not only were we able to quickly implement popular features from libraries such as [theme-ui], we also explored that <PackageLink name="uinix-ui`'s `Element` component behaves essentially as a configurable `Box` component.
